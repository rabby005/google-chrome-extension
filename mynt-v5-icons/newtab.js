...
// ═══════════════════════════════════════════════
// FAVICON — Google's favicon service (works in extensions)
// ═══════════════════════════════════════════════
...
function fallbackColor(str) {
  const C = ['#EA4335','#1A73E8','#34A853','#FBBC04','#FF6D00','#7B1FA2','#00897B','#E91E63','#FF5722','#607D8B','#4527A0','#00695C','#F57C00','#0288D1'];
  let h = 0;
  for (let i = 0; i < str.length; i++) h = (h * 31 + str.charCodeAt(i)) % C.length;
  return C[h];
}
...
const GAPPS = {
  ...
  cloud:[
    ...
    {l:'Search Console',u:'https://search.google.com/search-console',i:'https://fonts.gstatic.com/s/i/productlogos/search_console/v3/web-64dp/logo_search_console_color_1x_web_64dp.png'},
...
function loadST() {
  try { const s = localStorage.getItem('mynt6'); if(s) ST = Object.assign({}, DEF, JSON.parse(s)); } catch(e){}
}
function saveST() {
  try { localStorage.setItem('mynt6', JSON.stringify(ST)); } catch(e){}
}
...
// ═══════════════════════════════════════════════
// APP GRID — real favicons via Google S2
// ═══════════════════════════════════════════════
function makeApp(app, idx) {
  ...
  // Use direct icon URL if provided (Google apps), else favicon service
  const iconSrc = app.i || favicon(app.u);
  const fb = fallbackColor(app.l);
  const letter = app.l.charAt(0).toUpperCase();

  const ico = document.createElement('div');
  ico.className = 'app-ico';
  // White background for Google icons so they look clean
  ...
  img.onerror = function() {
    // Fallback to favicon service first, then colored letter
    if (app.i && this.src !== favicon(app.u)) {
      this.src = favicon(app.u);
      this.parentNode.style.background = '#fff';
    } else {
      this.parentNode.style.background = fb;
      this.parentNode.style.fontSize = '22px';
      this.parentNode.style.fontWeight = '700';
      this.parentNode.style.color = '#fff';
      this.parentNode.style.borderRadius = '14px';
...
function fillGrid(id, arr) {
  ...
  if (!el) return;
  el.innerHTML = '';
  arr.forEach((app, i) => el.appendChild(makeApp(app, i)));
}
...
async function loadBookmarks() {
  ...
  if (typeof chrome === 'undefined' || !chrome.bookmarks) {
    gridEl.innerHTML = '<div class="bm-empty"><div style="font-size:48px;opacity:.3">🔖</div><p>Chrome Bookmarks API not available.<br>Please reload the extension.</p></div>';
    return;
  }
  ...
  try {
    ...
    // Build folder list: top-level sections + their sub-folders
    chipsEl.innerHTML = '';

    // Render chips for top-level folders (Bookmarks Bar, Other, Mobile)
    rootChildren.forEach(section => {
      // Add section chip
      addFolderChip(chipsEl, section, true);
      // Add sub-folder chips indented
      if (section.children) {
        section.children.filter(n => !n.url).forEach(subFolder => {
          addFolderChip(chipsEl, subFolder, false, '  └ ');
        });
      }
    });

    // Show first section by default
    if (!activeFolderId && rootChildren.length > 0) {
      activeFolderId = rootChildren[0].id;
    }

    // Highlight active chip
    chipsEl.querySelectorAll('.bm-chip').forEach(c => {
      c.classList.toggle('on', c.dataset.id === activeFolderId);
    });

    // Render bookmarks for active folder
    await renderBmGrid(gridEl);

  } catch(err) {
    gridEl.innerHTML = `<div class="bm-empty"><div style="font-size:48px;opacity:.3">⚠️</div><p>Error: ${err.message}</p></div>`;
  }
}

function addFolderChip(container, folder, isTop, prefix='') {
  const chip = document.createElement('button');
  chip.className = 'bm-chip' + (folder.id === activeFolderId ? ' on' : '');
  chip.dataset.id = folder.id;
  chip.textContent = (isTop ? '📂 ' : '📁 ') + prefix + (folder.title || 'Unnamed');
  chip.onclick = () => {
...
async function renderBmGrid(gridEl) {
  gridEl.innerHTML = '<div class="bm-empty"><div style="font-size:32px">⏳</div><p>Loading...</p></div>';
  ...
  try {
    ...
    if (!folders.length && !bookmarks.length) {
      gridEl.innerHTML = '<div class="bm-empty"><div style="font-size:48px;opacity:.3">📭</div><p>This folder is empty.<br>Add bookmarks in Chrome or click <strong>Add Bookmark</strong>.</p></div>';
      return;
    }
    ...
    folders.forEach((f, i) => {
      ...
      card.innerHTML = `
        <div class="bm-fav" style="background:#FBBC04;font-size:20px;">📁</div>
        <div class="bm-info">
      ...
      card.onclick = () => {
        activeFolderId = f.id;
        document.querySelectorAll('.bm-chip').forEach(c=>c.classList.toggle('on', c.dataset.id===f.id));
        renderBmGrid(gridEl);
      };
    ...
    bookmarks.forEach((item, i) => {
      ...
      card.style.animationDelay = ((i+folders.length)*0.04)+'s';

      let host = item.url;
      try { host = new URL(item.url).hostname.replace('www.',''); } catch(e){}

      const fb = fallbackColor(item.title||host);
      const letter = (item.title||host||'?').charAt(0).toUpperCase();
      const fv = favicon(item.url);

      const favDiv = document.createElement('div');
      favDiv.className = 'bm-fav';
      const img = document.createElement('img');
      img.src = fv;
      img.alt = '';
      img.style.cssText = 'width:100%;height:100%;object-fit:contain;border-radius:6px;';
      img.onerror = function() {
        favDiv.style.background = fb;
        favDiv.style.display = 'flex';
        favDiv.style.alignItems = 'center';
        favDiv.style.justifyContent = 'center';
        favDiv.style.fontSize = '16px';
        favDiv.style.fontWeight = '700';
        favDiv.style.color = '#fff';
        favDiv.textContent = letter;
      };
      favDiv.appendChild(img);

      const infoDiv = document.createElement('div');
      infoDiv.className = 'bm-info';
      infoDiv.innerHTML = `<div class="bm-ttl">${item.title||host}</div><div class="bm-url">${host}</div>`;

      const delBtn = document.createElement('button');
      delBtn.className = 'bm-del';
      delBtn.textContent = '✕';
      delBtn.title = 'Remove from Chrome bookmarks';
      delBtn.onclick = async (e) => {
        e.preventDefault(); e.stopPropagation();
        if (confirm(`Remove "${item.title||host}" from bookmarks?`)) {
          await chrome.bookmarks.remove(item.id);
          snack('Removed from Chrome bookmarks');
          renderBmGrid(gridEl);
        }
      };
    ...
    function addOpts(nodes, depth) {
      nodes.forEach(n => {
        if (!n.url) {
          ...
          if (n.id === activeFolderId) o.selected = true;
          fSel.appendChild(o);
...
async function saveBm() {
  ...
  if (!title||!url) { snack('Please enter title and URL'); return; }
  if (!url.startsWith('http')) url = 'https://'+url;
  if (!parentId) { snack('Please select a folder'); return; }

  try {
    await chrome.bookmarks.create({parentId, title, url});
    snack('✅ Saved to Chrome Bookmarks!');
...
async function createFolder() {
  ...
  try {
    await chrome.bookmarks.create({parentId: activeFolderId || '1', title: name});
...
function init() {
  ...
  // ── ENGINE DROPDOWN ──
  document.getElementById('ebtn').addEventListener('click', e => {
    e.stopPropagation();
    document.getElementById('edd').classList.toggle('show');
  });
  ...
  document.getElementById('bm-new-fld').addEventListener('click', () => {
    ...
    setTimeout(() => document.getElementById('fld-mn').focus(), 100);
  });
