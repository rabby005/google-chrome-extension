...
let PT=null;
const f12=t=>{if(!t||!t.includes(':'))return t||'';const[h,m]=t.split(':').map(Number);return`${h%12||12}:${String(m).padStart(2,'0')} ${h>=12?'PM':'AM'}`;};
...
const CITY_COORDS={
  ...
  'khulna':{lat:22.8456,lon:89.5403},'london':{lat:51.5074,lon:-0.1278},
  'new york':{lat:40.7128,lon:-74.0060},'karachi':{lat:24.8607,lon:67.0011},
  'istanbul':{lat:41.0082,lon:28.9784},'cairo':{lat:30.0444,lon:31.2357},
  'riyadh':{lat:24.7136,lon:46.6753},'dubai':{lat:25.2048,lon:55.2708},
  'kuala lumpur':{lat:3.1390,lon:101.6869},'jakarta':{lat:-6.2088,lon:106.8456},
};
async function fetchP(city){
  const b=$('pw-body');if(!b)return;
  b.innerHTML='<div class="pw-loading">Loading…</div>';
  try{
    ...
    let url;
    if(coord){
      const method=ck==='dhaka'||ck==='chittagong'||ck==='sylhet'||ck==='rajshahi'||ck==='khulna'?1:2;
      url=`https://api.aladhan.com/v1/timings/${ds}?latitude=${coord.lat}&longitude=${coord.lon}&method=${method}`;
    }else{
      url=`https://api.aladhan.com/v1/timingsByCity/${ds}?city=${encodeURIComponent(city)}&country=&method=1`;
    }
    const r=await fetch(url);
    const d=await r.json();
    if(d.code===200&&d.data?.timings){
      PT=d.data.timings;renderP(city);
    }else{b.innerHTML='<div class="pw-loading">Could not load times</div>';}
...
function renderP(city){
  ...
  const fb=$('pw-fmt');
  if(fb){fb.textContent=S.hr12?'12H':'24H';fb.onclick=()=>{S.hr12=!S.hr12;saveS();fb.textContent=S.hr12?'12H':'24H';if($('tog-12h'))$('tog-12h').checked=S.hr12;document.querySelectorAll('.pw-rt[data-k]').forEach(e=>{if(PT)e.textContent=fT(PT[e.dataset.k]);});document.querySelectorAll('.pw-jt[data-j]').forEach(e=>{if(PT){const o=JO[e.dataset.j];if(o)e.textContent=fT(addM(PT[e.dataset.j],o));}});updateCD();};}
  updateCD();
}
function updateCD(){
  if(!PT)return;
  ...
  const fi=PK.indexOf(pK[ni]);
  document.querySelectorAll('.pw-row').forEach((r,i)=>r.classList.toggle('now',i===fi));
}
...
function init(){
  ...
  setInterval(()=>{const n=new Date();if(n.getHours()===0&&n.getMinutes()===0)fetchP(S.city);},60000);
