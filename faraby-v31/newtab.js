  ...
  const rows=PK.map((k,i)=>{
    ...
    return`<div class="pw-row" data-i="${i}">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:2px">
        ...
        ${jt?`<span class="pw-jt" data-j="${k}">জামাত ${fT(jt)}</span>`:''}
      </div>
    </div>`;
  }).join('');
  b.innerHTML=`
...
function updateCD(){
  ...
  // next prayer = first whose time > now
  ...
  if(nn)nn.textContent=pN[ni]+(wrapping?' ↻':'');
  ...
  document.querySelectorAll('.pw-row').forEach((r,i)=>r.classList.toggle('now',i===nextFullIdx));
}
