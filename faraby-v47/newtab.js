...
function updateCD(){
  ...
  if(ci>=0){
    // We are INSIDE a prayer window — show current prayer name in banner
    // until this prayer window ends (= next prayer starts)
    if(nn)nn.textContent=pN[ci]+' — চলছে';
    if(ct)ct.textContent=fT(PT[pK[ci]]);
    if(cd)cd.textContent=String(dh).padStart(2,'0')+':'+String(dm).padStart(2,'0')+' পর '+pN[ni];
  } else {
    // Before Fajr — show next prayer countdown
  ...
  // Highlight NEXT prayer row (the one coming up)
  const nextFullIdx = PK.indexOf(pK[ni]);
  // Current active prayer full index
  const currFullIdx = ci >= 0 ? PK.indexOf(pK[ci]) : -1;

  document.querySelectorAll('.pw-row').forEach((r, i) => {
    const isNext    = (i === nextFullIdx);
    const isCurrent = (i === currFullIdx);
    const isActive  = isNext || isCurrent;
    r.classList.toggle('next',    isNext);
    r.classList.toggle('current', isCurrent);
    // Show only active prayers, hide others
    r.style.display = isActive ? 'flex' : 'none';
  });

  // Update Dhuhr label in prayer list based on day
