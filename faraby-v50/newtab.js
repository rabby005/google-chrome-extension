    ...
    <div class="pw-banner">
      <div class="pw-bn-top">
        <div class="pw-bn-left">
          ...
          <div class="pw-bn-state" id="pw-bn-state"></div>
        </div>
        <div class="pw-bn-time-badge">
          <div class="pw-bn-clock" id="pw-ct">—</div>
          <div class="pw-bn-clock-lbl">সময়</div>
        </div>
      </div>
      <div class="pw-bn-countdown">
        <span class="pw-bn-cd-num" id="pw-cd">—</span>
        <span class="pw-bn-cd-lbl" id="pw-cd-lbl">বাকি আছে</span>
      </div>
    </div>
    <div class="pw-list">${rows}</div>
    <div style="padding:2px 10px 8px;font-size:9px;color:var(--t3);font-family:var(--m)">📍 ${city||S.city}</div>`;
  const fb=$('pw-fmt');
  if(fb){
...
function updateCD(){
  ...
  const bnLabel=$('pw-bn-label'), bnState=$('pw-bn-state'), cdLbl=$('pw-cd-lbl');
  ...
  if(ci>=0){
    ...
    if(bnLabel) bnLabel.textContent='এখন চলছে';
    if(nn)      nn.textContent=pN[ci];
    if(bnState) bnState.textContent='পরবর্তী: '+pN[ni];
    if(ct)      ct.textContent=fT(PT[pK[ci]]);
    if(cd)      cd.textContent=hh+':'+mm;
    if(cdLbl)   cdLbl.textContent='পর '+pN[ni]+' শুরু';
  } else {
    // Before first prayer of the day
    if(bnLabel) bnLabel.textContent='পরবর্তী নামাজ';
    if(nn)      nn.textContent=pN[ni];
    if(bnState) bnState.textContent='';
    if(ct)      ct.textContent=fT(PT[pK[ni]]);
    if(cd)      cd.textContent=hh+':'+mm;
    if(cdLbl)   cdLbl.textContent='বাকি আছে';
  }

  // Row states: done / current / next
  ...
  document.querySelectorAll('.pw-row').forEach((r, i) => {
    ...
    else if(i < nextFullIdx && i !== currFullIdx) r.classList.add('done');
  });
