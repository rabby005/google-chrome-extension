...
const PAL=[
  ...
  ,{a:'#003333',acl:'rgba(0,51,51,.12)',acb:'rgba(0,51,51,.30)'}
];
...
const DEF={
  ...
  customTxt:null,
  // Light-mode separate custom overrides
  lmCustomBg:null, lmCustomCardBg:null, lmCustomCardBd:null,
  lmCustomTxt:null, lmCustomTxt2:null, lmCustomTxt3:null,
  lmCustomSurf1:null, lmCustomSurf2:null,
  lmCustomAc:null,
  // Glass
  glass:false,glassBlur:12,glassOp:4,
  font:"'Jost',sans-serif",fs:14,br:14,navW:220,
  showQuotes:true,showWx:true,showCyber:false,showWebDev:true,
  gappCols:15,aiCols:15,bmCols:10,
...
const loadS=()=>{
  try{
    ...
    if(d){
      ...
      // showWebDev defaults to true — only respect saved value if explicitly set
      if(!('showWebDev' in saved)) S.showWebDev=true;
    }
  }catch(_){}
};
...
function sv(key, defKey) {
  // In light mode, use lmCustomX variant; in dark mode use customX
  var lmKey = 'lm' + key.charAt(0).toUpperCase() + key.slice(1);
  var v = S.lm ? (S[lmKey] !== null && S[lmKey] !== undefined && S[lmKey] !== '' ? S[lmKey] : null) : S[key];
  if (v === null || v === undefined || v === '') return modeDefaults()[defKey];
  return v;
}

/* Get the correct storage key for current mode */
function svKey(key) {
  if (!S.lm) return key;
  return 'lm' + key.charAt(0).toUpperCase() + key.slice(1);
}
...
function applyCustom() {
  ...
  // Card shadow
  var sBlur = (S.customCardShadowBlur  != null) ? S.customCardShadowBlur  : D.shBlur;
  var sY    = (S.customCardShadowY     != null) ? S.customCardShadowY     : D.shY;
  var sOp   = (S.customCardShadowOp   != null) ? S.customCardShadowOp   : D.shOp;
  var sCol  = S.customCardShadowColor || D.shCol;
  css('--card-shadow', buildShadow(sBlur, sY, sOp, sCol));

  // Text colours — all 3 levels (mode-aware via sv())
  css('--t1', sv('customTxt',  'txt'));
  css('--t2', sv('customTxt2', 'txt2'));
  css('--t3', sv('customTxt3', 'txt3'));

  // Surface colours (mode-aware via sv())
  var s1 = sv('customSurf1', 'surf1');
  var s2 = sv('customSurf2', 'surf2');
  var s3 = sv('customSurf3', 'surf3');
  css('--s1', s1); css('--c1', s1);
  css('--s2', s2); css('--c2', s2);
  css('--s3', s3); css('--c3', s3);

  // Light-mode specific system vars (acl, acb) that were in body.lm CSS
  if (S.lm) {
    css('--acl', 'rgba(77,127,255,.08)');
    css('--acb', 'rgba(77,127,255,.22)');
  } else {
    css('--acl', 'rgba(77,127,255,.1)');
    css('--acb', 'rgba(77,127,255,.28)');
  }
...
function applyTheme() {
  ...
  var activeAc = S.lm ? (S.lmCustomAc || S.customAc) : S.customAc;
  if (activeAc) setCustomAccent(activeAc);
  else          setAccent(S.pal || 0);
  applyCustom();
  applyGlass();
  syncColorPickers();
  // Update drawer mode indicator
  var modeNote = document.getElementById('dr-mode-note');
  if (modeNote) {
    modeNote.textContent = S.lm ? '☀️ Light Mode এ কাস্টমাইজ হচ্ছে' : '🌙 Dark Mode এ কাস্টমাইজ হচ্ছে';
    modeNote.style.color = S.lm ? '#c85000' : 'var(--ac)';
  }
}
...
function syncColorPickers() {
  ...
  /* sync native pickers */
  function syncNat(nId, v) { var n=$(nId); if(n && /^#[0-9a-fA-F]{6}$/.test(v)) n.value=v; }

  var acVal = (S.lm ? S.lmCustomAc : S.customAc) || (PAL[S.pal >= 0 ? S.pal : 0] || PAL[0]).a;
  setCP('color-hex',       'color-preview',      acVal);
  syncNat('ac-native', acVal);
  setCP('bg-hex',          'bg-preview',          sv('customBg',     'bg'));
  syncNat('bg-native',   sv('customBg',     'bg'));
  setCP('card-hex',        'card-preview',        sv('customCardBg', 'cardBg'));
  syncNat('card-native', sv('customCardBg', 'cardBg'));
  setCP('bd-hex',          'bd-preview',          sv('customCardBd', 'cardBd'));
  syncNat('bd-native',   sv('customCardBd', 'cardBd'));
  var t1v=sv('customTxt','txt'), t2v=sv('customTxt2','txt2'), t3v=sv('customTxt3','txt3');
  var s1v=sv('customSurf1','surf1'), s2v=sv('customSurf2','surf2');
  setCP('txt-hex',  'txt-preview',  t1v); syncNat('txt-native',   t1v);
  setCP('txt2-hex', 'txt2-preview', t2v); syncNat('txt2-native',  t2v);
  setCP('txt3-hex', 'txt3-preview', t3v); syncNat('txt3-native',  t3v);
  setCP('surf1-hex','surf1-preview',s1v); syncNat('surf1-native', s1v);
  setCP('surf2-hex','surf2-preview',s2v); syncNat('surf2-native', s2v);
  setCP('txt2-hex',        'txt2-preview',        sv('customTxt2',   'txt2'));
  setCP('txt3-hex',        'txt3-preview',        sv('customTxt3',   'txt3'));
  setCP('surf1-hex',       'surf1-preview',       sv('customSurf1',  'surf1'));
  setCP('surf2-hex',       'surf2-preview',       sv('customSurf2',  'surf2'));
...
function tick(){
  const _n=new Date();
  // Use city timezone if available for clock display
  let n=_n;
  try{if(PT_TZ){const ls=_n.toLocaleString('en-US',{timeZone:PT_TZ});n=new Date(ls);}}
  catch(e){}
...
let PT=null,PT_DATE='',PT_CITY='',PT_TZ=null;
...
const CITY_DB={
  'dhaka':{lat:23.8103,lon:90.4125,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'chittagong':{lat:22.3569,lon:91.7832,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'chattogram':{lat:22.3569,lon:91.7832,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'sylhet':{lat:24.8949,lon:91.8687,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'rajshahi':{lat:24.3745,lon:88.6042,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'khulna':{lat:22.8456,lon:89.5403,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'barisal':{lat:22.7010,lon:90.3535,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'barishal':{lat:22.7010,lon:90.3535,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'comilla':{lat:23.4607,lon:91.1809,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'mymensingh':{lat:24.7471,lon:90.4203,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'rangpur':{lat:25.7439,lon:89.2752,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'narayanganj':{lat:23.6238,lon:90.4996,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'gazipur':{lat:23.9999,lon:90.4203,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'bogura':{lat:24.8510,lon:89.3711,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'jessore':{lat:23.1634,lon:89.2182,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'jashore':{lat:23.1634,lon:89.2182,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'cox\'s bazar':{lat:21.4272,lon:92.0058,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'cox bazar':{lat:21.4272,lon:92.0058,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'tongi':{lat:23.8893,lon:90.4032,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'karachi':{lat:24.8607,lon:67.0011,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'lahore':{lat:31.5497,lon:74.3436,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'islamabad':{lat:33.6007,lon:73.0679,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'delhi':{lat:28.6139,lon:77.2090,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'new delhi':{lat:28.6139,lon:77.2090,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'mumbai':{lat:19.0760,lon:72.8777,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'kolkata':{lat:22.5726,lon:88.3639,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
  'hyderabad':{lat:17.3850,lon:78.4867,method:1,school:1,tune:'0,0,0,0,0,0,9,0,0'},
...
async function fetchP(city){
  ...
  try{
    ...
    // Build URL with optional tune corrections
    // For Bangladesh (method=1): Isha needs +adjustment for 18° instead of 17°
    let urlParams = `latitude=${coord.lat}&longitude=${coord.lon}&method=${coord.method}&school=${coord.school}`;
    if(coord.tune) urlParams += `&tune=${coord.tune}`;
    const url=`https://api.aladhan.com/v1/timings/${ds}?${urlParams}`;
    ...
    if(d.code===200&&d.data?.timings){
      PT={};
      // Strip timezone suffix but store raw for reference
      Object.keys(d.data.timings).forEach(k=>{PT[k]=stripTZ(d.data.timings[k]);});
      // Store the city timezone from API metadata (e.g. "Asia/Dhaka")
      PT_TZ = d.data?.meta?.timezone || null;
      PT_DATE=ds;
      PT_CITY=displayCity;
      renderP(displayCity);
    }else{
...
function renderP(city){
  ...
  b.innerHTML=`
    <div class="pw-banner">
      <div class="pw-bn-row1">
        <div>
          <div class="pw-bn-label" id="pw-bn-label">পরবর্তী নামাজ</div>
          <div class="pw-bn-name" id="pw-nn">—</div>
          <div class="pw-bn-sub" id="pw-bn-sub"></div>
        </div>
        <div class="pw-bn-badge">
          <div class="pw-bn-badge-time" id="pw-ct">—</div>
          <div class="pw-bn-badge-lbl">সময়</div>
        </div>
      </div>
      <div class="pw-bn-row2">
        <span class="pw-bn-cd" id="pw-cd">—</span>
        <span class="pw-bn-cd-txt" id="pw-cd-txt">বাকি আছে</span>
      </div>
    </div>
    <div class="pw-list">${rows}</div>
    <div style="padding:2px 12px 8px;font-size:9px;color:var(--t3);font-family:var(--m)">📍 ${city||S.city}</div>`;
  const fb=$('pw-fmt');
  if(fb){
...
function updateCD(){
  if(!PT)return;
  // Get current time in the PRAYER CITY's timezone (not browser timezone)
  // This fixes wrong countdown when browser is in different timezone
  let nowH, nowM, nowSec, isFri;
  try{
    if(PT_TZ){
      // Use the timezone from API metadata for accurate local time
      const cityNow = new Date(new Date().toLocaleString('en-US',{timeZone:PT_TZ}));
      nowH=cityNow.getHours(); nowM=cityNow.getMinutes(); nowSec=cityNow.getSeconds();
      isFri=cityNow.getDay()===5;
    } else {
      nowH=new Date().getHours(); nowM=new Date().getMinutes(); nowSec=new Date().getSeconds();
      isFri=new Date().getDay()===5;
    }
  } catch(e){
    nowH=new Date().getHours(); nowM=new Date().getMinutes(); nowSec=new Date().getSeconds();
    isFri=new Date().getDay()===5;
  }
  const nm = nowH*60 + nowM + nowSec/60;

  // Exclude Sunrise from countdown - it is not a salah
  const pK=["Fajr","Dhuhr","Asr","Maghrib","Isha"];
  const pN=["ফজর", isFri?"যোহর (জুমা)":"যোহর", "আসর","মাগরিব","ইশা"];
  const mins=pK.map(k=>toM(PT[k]));

  // Find NEXT upcoming prayer index
  ...
  const ds=Math.floor((diff%1)*60);

  // Find CURRENT prayer (last prayer whose time has passed)
  // ci = index of prayer currently active (time has passed but next hasn't started)
  let ci = -1;
  for(let i=mins.length-1;i>=0;i--){
    if(mins[i]<=nm){ ci=i; break; }
  }

  const hh = String(dh).padStart(2,'0');
  const mm_ = String(dm).padStart(2,'0');
  const cdStr = hh+':'+mm_;

  const nn     = $('pw-nn');
  const ct     = $('pw-ct');
  const cd     = $('pw-cd');
  const cdTxt  = $('pw-cd-txt');
  const bnLbl  = $('pw-bn-label');
  const bnSub  = $('pw-bn-sub');

  if(ci >= 0){
    // A prayer is currently active
    if(bnLbl) bnLbl.textContent = 'এখন চলছে';
    if(nn)    nn.textContent    = pN[ci];
    if(bnSub) bnSub.textContent = 'পরবর্তী › '+pN[ni];
    if(ct)    ct.textContent    = fT(PT[pK[ci]]);
    if(cd)    cd.textContent    = cdStr;
    if(cdTxt) cdTxt.textContent = 'পর '+pN[ni]+' শুরু';
  } else {
    // Before first prayer
    if(bnLbl) bnLbl.textContent = 'পরবর্তী নামাজ';
    if(nn)    nn.textContent    = pN[ni];
    if(bnSub) bnSub.textContent = '';
    if(ct)    ct.textContent    = fT(PT[pK[ni]]);
    if(cd)    cd.textContent    = cdStr;
    if(cdTxt) cdTxt.textContent = 'বাকি আছে';
  }

  // Row states
  const nextFullIdx = PK.indexOf(pK[ni]);
  const currFullIdx = ci >= 0 ? PK.indexOf(pK[ci]) : -1;

  document.querySelectorAll('.pw-row').forEach((r, i) => {
    r.classList.remove('next','current','done');
    r.style.display = 'flex';
    if(i === nextFullIdx)      r.classList.add('next');
    else if(i === currFullIdx) r.classList.add('current');
    else if(currFullIdx >= 0 && i < currFullIdx) r.classList.add('done');
    else if(currFullIdx < 0 && i < nextFullIdx)  r.classList.add('done');
  });
...
function setGappCols(v){
  S.gappCols=Math.max(2,Math.min(20,v));saveS();
...
function setAiCols(v){
  S.aiCols=Math.max(2,Math.min(20,v));saveS();
...
/* ══════════════════════════════════════════════
   WEB DEVELOPER HUB — Tool Data
══════════════════════════════════════════════ */
const WEBDEV_DATA = {

  ref: [
    {n:'MDN Web Docs',       u:'https://developer.mozilla.org',     tag:'HTML·CSS·JS Reference', color:'blue'},
    {n:'DevDocs',            u:'https://devdocs.io',                 tag:'All docs in one place',  color:'cyan'},
    {n:'W3Schools',          u:'https://w3schools.com',              tag:'Beginner friendly',       color:'green'},
    {n:'CSS Tricks',         u:'https://css-tricks.com',             tag:'CSS guides & tricks',    color:'purple'},
    {n:'Can I Use',          u:'https://caniuse.com',                tag:'Browser support check',  color:'orange'},
    {n:'HTML Reference',     u:'https://htmlreference.io',           tag:'Visual HTML guide',      color:'red'},
    {n:'CSS Reference',      u:'https://cssreference.io',            tag:'Visual CSS guide',       color:'blue'},
    {n:'JavaScript.info',    u:'https://javascript.info',            tag:'Modern JS tutorial',     color:'yellow'},
    {n:'web.dev',            u:'https://web.dev',                    tag:'Google best practices',  color:'green'},
    {n:'Smashing Magazine',  u:'https://smashingmagazine.com',       tag:'Web design articles',    color:'orange'},
  ],

  tools: [
    {n:'CodePen',            u:'https://codepen.io',                 tag:'HTML/CSS/JS playground',  color:'green'},
    {n:'JSFiddle',           u:'https://jsfiddle.net',               tag:'JS code playground',      color:'blue'},
    {n:'StackBlitz',         u:'https://stackblitz.com',             tag:'Full-stack IDE online',   color:'blue'},
    {n:'CodeSandbox',        u:'https://codesandbox.io',             tag:'React/Vue/Node sandbox',  color:'purple'},
    {n:'Replit',             u:'https://replit.com',                 tag:'Collaborative coding',    color:'orange'},
    {n:'Glitch',             u:'https://glitch.com',                 tag:'Build & deploy apps',     color:'pink'},
    {n:'Regex101',           u:'https://regex101.com',               tag:'Regex tester & debugger', color:'red'},
    {n:'JSON Formatter',     u:'https://jsonformatter.curiousconcept.com', tag:'Validate & format JSON', color:'cyan'},
    {n:'Prettier Playground',u:'https://prettier.io/playground',     tag:'Code formatter online',   color:'green'},
    {n:'Babel REPL',         u:'https://babeljs.io/repl',            tag:'JS transpiler tester',    color:'yellow'},
    {n:'TypeScript Playground',u:'https://typescriptlang.org/play',  tag:'TS compiler online',      color:'blue'},
    {n:'Vue SFC Playground', u:'https://play.vuejs.org',             tag:'Vue component tester',    color:'green'},
  ],

  css: [
    {n:'Flexbox Froggy',     u:'https://flexboxfroggy.com',          tag:'Learn Flexbox game',      color:'green'},
    {n:'Grid Garden',        u:'https://cssgridgarden.com',          tag:'Learn CSS Grid game',     color:'green'},
    {n:'CSS Grid Generator', u:'https://cssgrid-generator.netlify.app', tag:'Visual grid builder',  color:'blue'},
    {n:'Flexbox Generator',  u:'https://loading.io/flexbox',         tag:'Visual flex builder',     color:'cyan'},
    {n:'Gradient Generator', u:'https://cssgradient.io',             tag:'CSS gradient maker',      color:'purple'},
    {n:'Box Shadow Gen',     u:'https://box-shadow.dev',             tag:'Shadow CSS generator',    color:'blue'},
    {n:'Animista',           u:'https://animista.net',               tag:'CSS animations',          color:'pink'},
    {n:'Cubic Bezier',       u:'https://cubic-bezier.com',           tag:'Easing curve editor',     color:'orange'},
    {n:'Clippy',             u:'https://bennettfeely.com/clippy',    tag:'CSS clip-path maker',     color:'red'},
    {n:'Houdini Paint',      u:'https://houdini.how',                tag:'CSS Houdini worklets',    color:'purple'},
    {n:'CSS Filter Gen',     u:'https://cssgenerator.org/filter-css-generator.html', tag:'CSS filters', color:'cyan'},
    {n:'Neumorphism',        u:'https://neumorphism.io',             tag:'Soft UI generator',       color:'white'},
  ],

  color: [
    {n:'Coolors',            u:'https://coolors.co',                 tag:'Palette generator',       color:'orange'},
    {n:'Adobe Color',        u:'https://color.adobe.com',            tag:'Color wheel & themes',    color:'red'},
    {n:'Palette.app',        u:'https://paletteapp.xyz',             tag:'UI color palettes',       color:'purple'},
    {n:'Color Hunt',         u:'https://colorhunt.co',               tag:'Curated palettes',        color:'pink'},
    {n:'Realtime Colors',    u:'https://realtimecolors.com',         tag:'Live UI color preview',   color:'blue'},
    {n:'Contrast Checker',   u:'https://webaim.org/resources/contrastchecker', tag:'WCAG contrast', color:'green'},
    {n:'HexColor.co',        u:'https://hexcolor.co',                tag:'Hex color info',          color:'yellow'},
    {n:'UI Colors',          u:'https://uicolors.app',               tag:'Tailwind color shades',   color:'cyan'},
    {n:'Happy Hues',         u:'https://happyhues.co',               tag:'Color in context',        color:'orange'},
    {n:'Paletton',           u:'https://paletton.com',               tag:'Color scheme designer',   color:'purple'},
  ],

  fonts: [
    {n:'Google Fonts',       u:'https://fonts.google.com',           tag:'Free web fonts',          color:'blue'},
    {n:'Font Squirrel',      u:'https://fontsquirrel.com',           tag:'Free fonts',              color:'green'},
    {n:'Fontpair',           u:'https://fontpair.co',                tag:'Font pairing guide',      color:'purple'},
    {n:'Type Scale',         u:'https://typescale.com',              tag:'Modular type scale',      color:'orange'},
    {n:'Fontjoy',            u:'https://fontjoy.com',                tag:'AI font pairing',         color:'cyan'},
    {n:'Variable Fonts',     u:'https://v-fonts.com',                tag:'Variable font showcase',  color:'blue'},
    {n:'Bunny Fonts',        u:'https://fonts.bunny.net',            tag:'Privacy-friendly fonts',  color:'green'},
    {n:'Font Brief',         u:'https://www.fontbrief.com',          tag:'Font discovery',          color:'pink'},
  ],

  icons: [
    {n:'Iconify',            u:'https://iconify.design',             tag:'100k+ icons unified',     color:'blue'},
    {n:'Heroicons',          u:'https://heroicons.com',              tag:'Tailwind SVG icons',      color:'purple'},
    {n:'Lucide',             u:'https://lucide.dev',                 tag:'Beautiful SVG icons',     color:'cyan'},
    {n:'Phosphor Icons',     u:'https://phosphoricons.com',          tag:'Flexible icon family',    color:'yellow'},
    {n:'Feather Icons',      u:'https://feathericons.com',           tag:'Simply beautiful icons',  color:'green'},
    {n:'Tabler Icons',       u:'https://tabler-icons.io',            tag:'1500+ free SVG icons',    color:'blue'},
    {n:'Font Awesome',       u:'https://fontawesome.com',            tag:'Most popular icon set',   color:'blue'},
    {n:'Bootstrap Icons',    u:'https://icons.getbootstrap.com',     tag:'Official BS icons',       color:'purple'},
    {n:'SVGRepo',            u:'https://svgrepo.com',                tag:'500k+ free SVGs',         color:'orange'},
    {n:'Flaticon',           u:'https://flaticon.com',               tag:'Flat icon packs',         color:'red'},
    {n:'Simple Icons',       u:'https://simpleicons.org',            tag:'Brand SVG icons',         color:'white'},
    {n:'Lordicon',           u:'https://lordicon.com',               tag:'Animated icons',          color:'pink'},
  ],

  perf: [
    {n:'PageSpeed Insights', u:'https://pagespeed.web.dev',          tag:'Core Web Vitals score',   color:'green'},
    {n:'GTmetrix',           u:'https://gtmetrix.com',               tag:'Page speed analysis',     color:'orange'},
    {n:'WebPageTest',        u:'https://webpagetest.org',            tag:'Detailed perf testing',   color:'blue'},
    {n:'Lighthouse',         u:'https://googlechrome.github.io/lighthouse/viewer', tag:'Perf/A11y audit', color:'yellow'},
    {n:'Pingdom',            u:'https://tools.pingdom.com',          tag:'Uptime & speed monitor',  color:'purple'},
    {n:'Bundlephobia',       u:'https://bundlephobia.com',           tag:'npm package size',        color:'red'},
    {n:'Squoosh',            u:'https://squoosh.app',                tag:'Image compression',       color:'pink'},
    {n:'SVGOMG',             u:'https://svgomg.net',                 tag:'SVG optimizer',           color:'cyan'},
    {n:'ImageOptim',         u:'https://imageoptim.com',             tag:'Image optimizer',         color:'green'},
    {n:'Lighthouse CI',      u:'https://github.com/GoogleChrome/lighthouse-ci', tag:'Automate audits', color:'yellow'},
  ],

  api: [
    {n:'Postman',            u:'https://postman.com',                tag:'API testing & docs',      color:'orange'},
    {n:'Hoppscotch',         u:'https://hoppscotch.io',              tag:'Open source API client',  color:'green'},
    {n:'JSONPlaceholder',    u:'https://jsonplaceholder.typicode.com',tag:'Fake REST API',           color:'blue'},
    {n:'Public APIs',        u:'https://publicapis.dev',             tag:'Free public API list',    color:'cyan'},
    {n:'RapidAPI',           u:'https://rapidapi.com',               tag:'API marketplace',         color:'blue'},
    {n:'Swagger UI',         u:'https://editor.swagger.io',          tag:'API design (OpenAPI)',    color:'green'},
    {n:'MockAPI',            u:'https://mockapi.io',                 tag:'Mock REST API',           color:'purple'},
    {n:'Insomnia',           u:'https://insomnia.rest',              tag:'REST/GraphQL client',     color:'purple'},
    {n:'HTTP Status Cats',   u:'https://http.cat',                   tag:'HTTP status reference 🐱',color:'pink'},
    {n:'httpbin',            u:'https://httpbin.org',                tag:'HTTP request inspector',  color:'blue'},
  ],

  deploy: [
    {n:'Vercel',             u:'https://vercel.com',                 tag:'Frontend cloud deploy',   color:'white'},
    {n:'Netlify',            u:'https://netlify.com',                tag:'Static site hosting',     color:'cyan'},
    {n:'GitHub Pages',       u:'https://pages.github.com',           tag:'Free static hosting',     color:'white'},
    {n:'Render',             u:'https://render.com',                 tag:'Cloud hosting platform',  color:'purple'},
    {n:'Railway',            u:'https://railway.app',                tag:'Deploy in seconds',       color:'purple'},
    {n:'Cloudflare Pages',   u:'https://pages.cloudflare.com',       tag:'Edge deployment',         color:'orange'},
    {n:'Supabase',           u:'https://supabase.com',               tag:'Open source Firebase',    color:'green'},
    {n:'Firebase',           u:'https://firebase.google.com',        tag:'Google app platform',     color:'yellow'},
    {n:'PlanetScale',        u:'https://planetscale.com',            tag:'MySQL serverless DB',     color:'white'},
    {n:'Neon',               u:'https://neon.tech',                  tag:'Serverless Postgres',     color:'green'},
    {n:'Upstash',            u:'https://upstash.com',                tag:'Serverless Redis/Kafka',  color:'green'},
    {n:'Deno Deploy',        u:'https://deno.com/deploy',            tag:'Serverless JS hosting',   color:'white'},
  ],

  learn: [
    {n:'freeCodeCamp',       u:'https://freecodecamp.org',           tag:'Free coding bootcamp',    color:'green'},
    {n:'The Odin Project',   u:'https://theodinproject.com',         tag:'Full stack curriculum',   color:'red'},
    {n:'Frontend Masters',   u:'https://frontendmasters.com',        tag:'Expert-led courses',      color:'red'},
    {n:'Scrimba',            u:'https://scrimba.com',                tag:'Interactive courses',     color:'purple'},
    {n:'CSS for JS Devs',    u:'https://css-for-js.dev',             tag:'CSS deep dive course',    color:'blue'},
    {n:'Josh W Comeau Blog', u:'https://joshwcomeau.com',            tag:'CSS/React deep dives',    color:'pink'},
    {n:'Kent C. Dodds',      u:'https://kentcdodds.com',             tag:'React testing & patterns',color:'blue'},
    {n:'Fireship.io',        u:'https://fireship.io',                tag:'Fast-paced web tutorials',color:'orange'},
    {n:'roadmap.sh',         u:'https://roadmap.sh',                 tag:'Dev learning roadmaps',   color:'green'},
    {n:'Patterns.dev',       u:'https://patterns.dev',               tag:'JS design patterns',      color:'cyan'},
  ],

  git: [
    {n:'GitHub',             u:'https://github.com',                 tag:'Code hosting',            color:'white'},
    {n:'GitLab',             u:'https://gitlab.com',                 tag:'DevOps platform',         color:'orange'},
    {n:'Conventional Commits',u:'https://conventionalcommits.org',   tag:'Commit message spec',     color:'blue'},
    {n:'Git Explorer',       u:'https://gitexplorer.com',            tag:'Find the right git cmd',  color:'purple'},
    {n:'GitHub CLI',         u:'https://cli.github.com',             tag:'GitHub in terminal',      color:'white'},
    {n:'Gitignore.io',       u:'https://gitignore.io',               tag:'.gitignore generator',    color:'green'},
    {n:'Semantic Versioning',u:'https://semver.org',                 tag:'Version spec guide',      color:'blue'},
    {n:'Mermaid Live',       u:'https://mermaid.live',               tag:'Diagram as code',         color:'cyan'},
    {n:'Excalidraw',         u:'https://excalidraw.com',             tag:'Collaborative diagrams',  color:'purple'},
  ],
};
...
function renderWebDev(){
  const sections={
    ref:'wd-ref', tools:'wd-tools', css:'wd-css', color:'wd-color',
    fonts:'wd-fonts', icons:'wd-icons', perf:'wd-perf',
    api:'wd-api', deploy:'wd-deploy', learn:'wd-learn', git:'wd-git'
  };
  Object.entries(sections).forEach(([key,gridId])=>{
    const grid=$(gridId); if(!grid)return;
    grid.innerHTML='';
    (WEBDEV_DATA[key]||[]).forEach(item=>{
      let host='';
      try{host=new URL(item.u).hostname;}catch(e){host=item.u;}
      const a=document.createElement('a');
      a.className=`cy-card tag-${item.color||'blue'}`;
      ...
      const img=document.createElement('img');
      img.src=`https://www.google.com/s2/favicons?domain=${host}&sz=64`;
      img.alt=''; img.width=26; img.height=26;
      img.style.cssText='width:26px;height:26px;object-fit:contain;border-radius:5px;display:block;';
      img.onerror=function(){
        this.remove();
        ico.style.background=colr(item.n);
...
function applyWebDevVisibility(){
  const show=(S.showWebDev===true||S.showWebDev===undefined||S.showWebDev===null);
  const tb=$('tab-btn-webdev'); if(tb)tb.style.display=show?'':'none';
  const nb=$('nav-btn-webdev'); if(nb)nb.style.display=show?'':'none';
  if(!show&&S.tab==='webdev')switchTab('google');
  const tog=$('tog-webdev'); if(tog)tog.checked=show;
}
...
function setBmCols(v){
  S.bmCols=Math.max(2,Math.min(20,v));saveS();
...
async function bmRenderGrid(){
  ...
  items.forEach(item=>{
    ...
    editOpt.className='bm-menu-item';
    editOpt.innerHTML='<span class="bm-mi-ico">&#9998;</span><span>Edit</span>';

    const delOpt=document.createElement('button');
    delOpt.className='bm-menu-item bm-menu-del';
    delOpt.innerHTML='<span class="bm-mi-ico">&#128465;</span><span>Delete</span>';

    editOpt.addEventListener('click', async e=>{
      e.preventDefault(); e.stopPropagation();
      closeAllBmMenus();
      // fill folder select
      const sel=$('bm-f'); if(!sel) return;
      sel.innerHTML='';
      if(typeof chrome!=='undefined' && chrome.bookmarks){
        try{
          const tree=await chrome.bookmarks.getTree();
          const fillSel=(nodes,depth)=>{
            nodes.forEach(n=>{
              if(!n.url){
                const o=document.createElement('option');
                o.value=n.id;
                o.textContent='　'.repeat(depth)+(n.title||'Bookmarks');
                if(n.id===item.parentId) o.selected=true;
                sel.appendChild(o);
                if(n.children) fillSel(n.children, depth+1);
              }
            });
          };
          fillSel(tree[0].children||[], 0);
        }catch(err){}
      }
      $('bm-t').value=title;
      $('bm-u').value=item.url;
...
async function saveBm(){
  ...
  try{
    if(editId){
      ...
      await chrome.bookmarks.update(editId,{title:t,url:u});
      // move to new folder if changed
      const bm=(await chrome.bookmarks.get(editId))[0];
      if(bm.parentId!==pid)await chrome.bookmarks.move(editId,{parentId:pid});
...
function closeAllBmMenus(){
  document.querySelectorAll('.bm-menu.open').forEach(m=>m.classList.remove('open'));
}

function closeBmModal(){
  const m=$('bm-modal');
  if(m)delete m.dataset.editId;
  $('bm-modal')?.classList.remove('open');
...
function switchTab(name){
  ...
  if(name==='webdev')renderWebDev();
}

/* RIGHT PANEL TABS */

/* ══════════════════════════════════════════════
   ARABIC / HIJRI CALENDAR ENGINE
══════════════════════════════════════════════ */

/* Hijri month names in Arabic + Bengali */
const HIJRI_MONTHS = [
  {ar:'مُحَرَّم',    bn:'মুহাররম'},
  {ar:'صَفَر',      bn:'সফর'},
  {ar:'رَبِيعُ الأَوَّل', bn:'রবিউল আউয়াল'},
  {ar:'رَبِيعُ الثَّانِي',bn:'রবিউস সানি'},
  {ar:'جُمَادَى الأُولَى',bn:'জমাদিউল আউয়াল'},
  {ar:'جُمَادَى الثَّانِيَة',bn:'জমাদিউস সানি'},
  {ar:'رَجَب',      bn:'রজব'},
  {ar:'شَعْبَان',   bn:'শাবান'},
  {ar:'رَمَضَان',   bn:'রমজান'},
  {ar:'شَوَّال',    bn:'শাওয়াল'},
  {ar:'ذُو الْقَعْدَة',bn:'জিলকদ'},
  {ar:'ذُو الْحِجَّة',bn:'জিলহজ'},
];

/* Special Islamic days: [month(1-based), day, name] */
const ISLAMIC_EVENTS = [
  [1,  1,  'মুহাররম শুরু'],
  [1,  10, 'আশুরা'],
  [3,  12, 'ঈদে মিলাদুন্নবি ﷺ'],
  [7,  27, 'মেরাজুন নবি'],
  [8,  15, 'শাবান-এ-বরাত'],
  [9,  1,  'রমজান শুরু'],
  [9,  21, 'শবে কদর?'],
  [9,  23, 'শবে কদর?'],
  [9,  25, 'শবে কদর?'],
  [9,  27, '⭐ শবে কদর (সম্ভাব্য)'],
  [9,  29, 'রমজান শেষ?'],
  [10, 1,  '🎊 ঈদুল ফিতর'],
  [12, 9,  'আরাফাত দিবস'],
  [12, 10, '🎉 ঈদুল আজহা'],
  [12, 11, 'আইয়ামে তাশরিক'],
  [12, 12, 'আইয়ামে তাশরিক'],
  [12, 13, 'আইয়ামে তাশরিক'],
];

/* Gregorian → Hijri converter (Kuwaiti algorithm) */
function gregToHijri(year, month, day) {
...
/* Days in a Hijri month (approximation using 30/29 alternating) */
function hijriDaysInMonth(hYear, hMonth) {
  if(hMonth===12){
    // Leap year check (30 year cycle)
    const leap=[2,5,7,10,13,15,18,21,24,26,29];
    return leap.includes(hYear%30)?30:29;
  }
  return hMonth%2===1?30:29;
}

/* Hijri month start — find the Gregorian date of Hijri month day 1 */
function hijriMonthStart(hYear, hMonth) {
  // Approximate: convert hijri 1st to Gregorian using inverse
  // Use binary search: find the greg date where gregToHijri = {hYear,hMonth,1}
  // Rough starting estimate: Hijri year 1 = ~622 CE July 16
  const EPOCH = 622.3;
  const approxYear = Math.floor(EPOCH + (hYear-1)*354.367/365.25 + (hMonth-1)*29.53/30);
  const approxDay  = new Date(approxYear, 0, 1);
  // Scan forward/backward from estimate
  let d = new Date(approxDay);
  d.setFullYear(approxYear);
  d.setMonth(0); d.setDate(1);
  // Scan ±60 days from estimate
  for(let i=-60;i<=60;i++){
    const test=new Date(d.getTime()+i*86400000);
    const h=gregToHijri(test.getFullYear(),test.getMonth()+1,test.getDate());
    if(h.y===hYear&&h.m===hMonth&&h.d===1) return test;
  }
  return null;
}

let acalHijriMonth = null;  // {y,m} currently displayed
let acalHijriYear  = null;

function initArabicCalendar() {
  const today = new Date();
  const h = gregToHijri(today.getFullYear(), today.getMonth()+1, today.getDate());
  acalHijriMonth = h.m;
  acalHijriYear  = h.y;
  renderArabicCalendar();
}

function renderArabicCalendar() {
  const cont = $('arabic-cal');
  if(!cont) return;

  const today      = new Date();
  const todayHijri = gregToHijri(today.getFullYear(), today.getMonth()+1, today.getDate());
  const hm = acalHijriMonth;
  const hy = acalHijriYear;
  const mn = HIJRI_MONTHS[hm-1];

  // Find Gregorian date of 1st of this Hijri month
  const monthStart = hijriMonthStart(hy, hm);
  const daysInMonth = hijriDaysInMonth(hy, hm);

  // Events for this month
  const monthEvents = ISLAMIC_EVENTS.filter(e=>e[0]===hm);

  // Today banner
  const isCurrentMonth = (todayHijri.y===hy && todayHijri.m===hm);
  const gregMonths=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const gregDays=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const todayGreg = `${today.getDate()} ${gregMonths[today.getMonth()]} ${today.getFullYear()}`;

  let html = '';

  // Header nav
  html += `<div class="acal-header">
    <button class="acal-nav" id="acal-prev">‹</button>
    <div class="acal-title-block">
      <div class="acal-hijri-month">${mn.ar}</div>
      <div class="acal-hijri-bn">${mn.bn}</div>
      <div class="acal-hijri-year">${hy} হিজরি</div>
      <div class="acal-greg-label">${monthStart ? monthStart.toLocaleString('en',{month:'short',year:'numeric'}) : ''}</div>
    </div>
    <button class="acal-nav" id="acal-next">›</button>
  </div>`;

  // Today banner (only in current month)
  if(isCurrentMonth){
    html += `<div class="acal-today-banner">
      <div class="acal-today-hijri">${todayHijri.d}</div>
      <div class="acal-today-right">
        <div class="acal-today-name">${mn.ar}</div>
        <div class="acal-today-bn">${mn.bn} ${todayHijri.y}H</div>
        <div class="acal-today-greg">${todayGreg}</div>
      </div>
    </div>`;
  }

  // DOW row — start from Sunday
  const DOWS=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  html += '<div class="acal-dow-row">';
  DOWS.forEach((d,i)=>html+=`<div class="acal-dow${i===5?' fri':''}">${d}</div>`);
  html += '</div>';

  // Calendar grid
  html += '<div class="acal-grid">';
  // Starting weekday of month
  const startDow = monthStart ? monthStart.getDay() : 0;
  // Empty cells
  for(let i=0;i<startDow;i++) html+=`<div class="acal-day empty"></div>`;

  for(let hDay=1;hDay<=daysInMonth;hDay++){
    const gregDate = monthStart ? new Date(monthStart.getTime()+(hDay-1)*86400000) : null;
    const dow = gregDate ? gregDate.getDay() : -1;
    const isFri = dow===5;
    const isToday = isCurrentMonth && hDay===todayHijri.d;
    const isSpecial = ISLAMIC_EVENTS.some(e=>e[0]===hm&&e[1]===hDay);
    const gregDay = gregDate ? gregDate.getDate() : '';

    let cls = 'acal-day';
    if(isToday) cls+=' today';
    if(isFri)   cls+=' is-friday';
    if(isSpecial&&!isToday) cls+=' special';

    html+=`<div class="${cls}" title="${isSpecial?ISLAMIC_EVENTS.find(e=>e[0]===hm&&e[1]===hDay)?.[2]:''}">
      <div class="acal-greg-day">${gregDay}</div>
      <div class="acal-h-day">${hDay}</div>
    </div>`;
  }
  html += '</div>';
  ...
  if(monthEvents.length){
    html += '<div class="acal-events">';
    monthEvents.forEach(([,,name],idx)=>{
      html+=`<div class="acal-event-item"><div class="acal-event-day">${monthEvents[idx][1]}</div><div class="acal-event-name">${name}</div></div>`;
    });
    html += '</div>';
  }
  ...
  // Wire nav buttons
  const prevBtn = $('acal-prev'), nextBtn = $('acal-next');
  if(prevBtn) prevBtn.addEventListener('click', ()=>{ acalNavMonth(-1); });
  if(nextBtn) nextBtn.addEventListener('click', ()=>{ acalNavMonth(1); });
}

function acalNavMonth(dir) {
  acalHijriMonth += dir;
  if(acalHijriMonth < 1)  { acalHijriMonth=12; acalHijriYear--; }
  if(acalHijriMonth > 12) { acalHijriMonth=1;  acalHijriYear++; }
  renderArabicCalendar();
}
...
function initRpTabs(){
  document.querySelectorAll('.rp-tab-btn').forEach(b=>{
    b.addEventListener('click',()=>{
      ...
      if(t==='rp-arabic')renderArabicCalendar();
...
function init(){
  ...
  css('--gapp-cols',S.gappCols||15);
  css('--bm-cols',S.bmCols||10);
  ['gapp-cols-val','s-gapp-val'].forEach(id=>{const e=$(id);if(e)e.textContent=S.gappCols||15;});
  ['ai-cols-val','s-ai-val'].forEach(id=>{const e=$(id);if(e)e.textContent=S.aiCols||15;});
  ['bm-cols-val','s-bm-val'].forEach(id=>{const e=$(id);if(e)e.textContent=S.bmCols||10;});
  ...
  document.addEventListener('click',e=>{
    if(!e.target.closest('.bm-dots')&&!e.target.closest('.bm-menu'))closeAllBmMenus();
  });
  ...
  if($('tog-webdev'))$('tog-webdev').checked=S.showWebDev!==false;
  applyCyberVisibility();
  applyWebDevVisibility();
  ...
  /* Wire native <input type="color"> to hex input + preview */
function wireNative(nativeId, hexId, prevId) {
  var nat = $(nativeId);
  if (!nat) return;
  nat.addEventListener('input', function() {
    var v = nat.value;
    var h = $(hexId); var p = $(prevId);
    if (h) h.value = v;
    if (p) p.style.background = v;
  });
  // sync native from hex on open
  var h = $(hexId);
  if (h) h.addEventListener('input', function() {
    var v = h.value;
    if (/^#[0-9a-fA-F]{6}$/.test(v)) { nat.value = v; }
    var p = $(prevId); if (p) p.style.background = v;
  });
}
...
function wirePicker(hexId, prevId, applyId, resetId, onApply, onReset) {
  ...
  wirePicker('color-hex', 'color-preview', 'color-apply', null,
    function(v) {
      S.pal = -1; if (S.lm) S.lmCustomAc = v; else S.customAc = v;
  ...
  wirePicker('bg-hex', 'bg-preview', 'bg-apply', 'bg-reset',
    function(v) { S[svKey('customBg')] = v; applyCustom(); saveS(); syncColorPickers(); snack('🖼 Background set'); },
    function()  { S[svKey('customBg')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Background reset'); }
  );

  /* Card Color */
  wirePicker('card-hex', 'card-preview', 'card-apply', 'card-reset',
    function(v) { S[svKey('customCardBg')] = v; applyCustom(); saveS(); syncColorPickers(); snack('🃏 Card color set'); },
    function()  { S[svKey('customCardBg')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Card color reset'); }
  );

  /* Card Border */
  wirePicker('bd-hex', 'bd-preview', 'bd-apply', 'bd-reset',
    function(v) { S[svKey('customCardBd')] = v; applyCustom(); saveS(); syncColorPickers(); snack('🔲 Border color set'); },
    function()  { S[svKey('customCardBd')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Border reset'); }
  );

  /* Primary Text */
  wirePicker('txt-hex', 'txt-preview', 'txt-apply', 'txt-reset',
    function(v) { S[svKey('customTxt')] = v; applyCustom(); saveS(); syncColorPickers(); snack('✏ Text color set'); },
    function()  { S[svKey('customTxt')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Text reset'); }
  );

  /* Secondary Text */
  wirePicker('txt2-hex', 'txt2-preview', 'txt2-apply', 'txt2-reset',
    function(v) { S[svKey('customTxt2')] = v; applyCustom(); saveS(); syncColorPickers(); snack('✏ Secondary text set'); },
    function()  { S[svKey('customTxt2')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Secondary text reset'); }
  );

  /* Muted Text */
  wirePicker('txt3-hex', 'txt3-preview', 'txt3-apply', 'txt3-reset',
    function(v) { S[svKey('customTxt3')] = v; applyCustom(); saveS(); syncColorPickers(); snack('✏ Muted text set'); },
    function()  { S[svKey('customTxt3')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Muted text reset'); }
  );

  /* Surface 1 (panels/nav bg) */
  wirePicker('surf1-hex', 'surf1-preview', 'surf1-apply', 'surf1-reset',
    function(v) { S[svKey('customSurf1')] = v; applyCustom(); saveS(); syncColorPickers(); snack('Surface 1 set'); },
    function()  { S[svKey('customSurf1')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Surface 1 reset'); }
  );

  /* Surface 2 (card/input bg) */
  wirePicker('surf2-hex', 'surf2-preview', 'surf2-apply', 'surf2-reset',
    function(v) { S[svKey('customSurf2')] = v; applyCustom(); saveS(); syncColorPickers(); snack('Surface 2 set'); },
    function()  { S[svKey('customSurf2')] = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Surface 2 reset'); }
  );

  /* Wire all native color pickers */
  wireNative('ac-native',    'color-hex', 'color-preview');
  wireNative('bg-native',    'bg-hex',    'bg-preview');
  wireNative('card-native',  'card-hex',  'card-preview');
  wireNative('bd-native',    'bd-hex',    'bd-preview');
  wireNative('txt-native',   'txt-hex',   'txt-preview');
  wireNative('txt2-native',  'txt2-hex',  'txt2-preview');
  wireNative('txt3-native',  'txt3-hex',  'txt3-preview');
  wireNative('surf1-native', 'surf1-hex', 'surf1-preview');
  wireNative('surf2-native', 'surf2-hex', 'surf2-preview');
  ...
  if($('tog-webdev'))$('tog-webdev').addEventListener('change',e=>{S.showWebDev=e.target.checked;saveS();applyWebDevVisibility();if(S.showWebDev)snack('🌐 Web Developer tab enabled');else snack('Web Developer tab hidden');});
  ...
  if($('bm-cancel'))$('bm-cancel').addEventListener('click',()=>{
    const m=$('bm-modal');
    if(m)delete m.dataset.editId;
    $('bm-modal').classList.remove('open');
    $('panelOv').classList.remove('open');
