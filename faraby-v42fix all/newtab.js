...
function syncColorPickers() {
  ...
  var acVal = S.customAc || (PAL[S.pal >= 0 ? S.pal : 0] || PAL[0]).a;
...
function setBmCols(v){
  S.bmCols=Math.max(1,Math.min(20,v));saveS();
...
function wirePicker(hexId, prevId, applyId, resetId, onApply, onReset) {
  ...
  wirePicker('txt-hex', 'txt-preview', 'txt-apply', 'txt-reset',
    ...
    function()  { S.customTxt = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Text reset'); }
  );

  /* Secondary Text */
  wirePicker('txt2-hex', 'txt2-preview', 'txt2-apply', 'txt2-reset',
    function(v) { S.customTxt2 = v; applyCustom(); saveS(); syncColorPickers(); snack('✏ Secondary text set'); },
    function()  { S.customTxt2 = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Secondary text reset'); }
  );

  /* Muted Text */
  wirePicker('txt3-hex', 'txt3-preview', 'txt3-apply', 'txt3-reset',
    function(v) { S.customTxt3 = v; applyCustom(); saveS(); syncColorPickers(); snack('✏ Muted text set'); },
    function()  { S.customTxt3 = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Muted text reset'); }
  );

  /* Surface 1 (panels/nav bg) */
  wirePicker('surf1-hex', 'surf1-preview', 'surf1-apply', 'surf1-reset',
    function(v) { S.customSurf1 = v; applyCustom(); saveS(); syncColorPickers(); snack('Surface 1 set'); },
    function()  { S.customSurf1 = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Surface 1 reset'); }
  );

  /* Surface 2 (card/input bg) */
  wirePicker('surf2-hex', 'surf2-preview', 'surf2-apply', 'surf2-reset',
    function(v) { S.customSurf2 = v; applyCustom(); saveS(); syncColorPickers(); snack('Surface 2 set'); },
    function()  { S.customSurf2 = null; applyCustom(); saveS(); syncColorPickers(); snack('↺ Surface 2 reset'); }
  );
