...
sidebarTabs.forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        // Remove active class from all sidebar tabs and categories
...
if (aiSearchBox) {
    aiSearchBox.addEventListener('input', (e) => {
        const query = e.target.value.toLowerCase().trim();
        const aiApps = document.querySelectorAll('.ai-grid .app-icon');
        ...
        // If empty, show all
        if (query === '') {
            aiApps.forEach(app => app.style.display = 'flex');
        }
    });
}
