...
function updateCD(){
  ...
  // Banner: show "পরবর্তী নামাজ" only when next prayer is within 60 min
  const pwBn = document.querySelector('.pw-banner');
  if(pwBn) pwBn.style.display = diff <= 60 ? '' : 'none';
  ...
  // Highlight next prayer row — all rows visible
  const nextFullIdx = PK.indexOf(pK[ni]);

  document.querySelectorAll('.pw-row').forEach((r, i) => {
    r.classList.toggle('next', i === nextFullIdx);
