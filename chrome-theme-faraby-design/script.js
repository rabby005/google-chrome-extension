...
chrome.storage.sync.get(['wallpaper', 'color1', 'color2'], function(result) {
    if (result.wallpaper) {
        ...
        // Hide orbs when custom wallpaper is set
    ...
    if (result.color1 && result.color2) {
        ...
        // Hide orbs when custom gradient is set
        document.querySelectorAll('.glow-orb').forEach(orb => orb.style.display = 'none');
    }
});
...
function applyGradientColors(c1, c2) {
    ...
    // Hide orbs when gradient is applied
    document.querySelectorAll('.glow-orb').forEach(orb => orb.style.display = 'none');
    // Remove default background effects
    wallpaperContainer.style.setProperty('background', `linear-gradient(135deg, ${c1} 0%, ${c2} 100%)`, 'important');
    const before = document.createElement('style');
    before.textContent = `#wallpaper-container::before, #wallpaper-container::after { display: none !important; }`;
...
resetWallpaperBtn.addEventListener('click', () => {
    wallpaperContainer.style.backgroundImage = '';
    wallpaperContainer.style.background = '#061a1a';
    // Show orbs again
    document.querySelectorAll('.glow-orb').forEach(orb => orb.style.display = 'block');
    // Remove any inline style overrides
    const styleOverrides = document.querySelectorAll('style');
    styleOverrides.forEach(style => {
        if (style.textContent.includes('#wallpaper-container::before')) {
            style.remove();
        }
    });
    chrome.storage.sync.remove(['wallpaper', 'color1', 'color2'], () => {
        ...
        alert('Reset to default Faraby design! ✓');
        modal.style.display = 'none';
    });
});
...
resetColorsBtn.addEventListener('click', () => {
    ...
    color1Input.value = '#667eea';
    color2Input.value = '#764ba2';
    chrome.storage.sync.set({ color1: null, color2: null, wallpaper: null }, () => {
        alert('Reset to default Faraby design! ✓');
    });
});
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape' && addBookmarkModal.style.display === 'block') {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'd') {
        e.preventDefault();
        const bookmarksActive = document.querySelector('.app-category[data-category="bookmarks"]').classList.contains('active');
        if (bookmarksActive) {
...
const closeBookmarkModalBtn = document.getElementById('closeBookmarkModal');
...
function renderBookmarks() {
    ...
    bookmarks.forEach((bookmark, index) => {
        ...
        const iconHTML = bookmark.iconUrl 
            ? `<img src="${bookmark.iconUrl}" alt="${bookmark.name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
               <svg style="display:none;" width="35" height="35" viewBox="0 0 24 24" fill="white">
        ...
        bookmarkEl.innerHTML = `
            ...
            <div class="delete-bookmark" data-index="${index}">
...
colorPreset.addEventListener('change', (e) => {
    if (e.target.value) {
        bookmarkColor.value = e.target.value;
    }
});
...
saveBookmarkBtn.addEventListener('click', () => {
    ...
    const iconUrl = document.getElementById('bookmarkIcon').value.trim();
